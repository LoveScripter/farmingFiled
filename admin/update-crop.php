...
if (count($_POST)>0) {
    ...
    if ($is_error==false) {
        ...
        if (!empty($uploaded_image_name)) {
            ...
            $image_query = ", cover_image = '$final_image'";
            
        }

        // lets build the sql query now to insert data into table.
        $sql = "UPDATE product SET  product_name='$name',product_descp = '$descp' $image_query   WHERE id=" . $product_id;
        ...
        if ($db_response) {
            ...
            header("Location:list-crop.php?success=Record update Successfully");
...
?>
 <div class="content-area">
                    ...
                    <div>
                        <form action="update-crop.php" method="POST" enctype="multipart/form-data">
                            <div class="table">
                                <div class="tr">
                                    <div class="cell"><strong>Name:</strong></div>
                                    <div class="cell"><input name="product_name" type="text" value="<?php echo $category_info['product_name']; ?>"/> </div>
                                </div>
                                <div class="tr">
                                    <div class="cell"><strong>Description:</strong> </div>
                                    <div class="cell"><textarea class="rich-editor"  name="product_descp"><?php echo $category_info['product_descp']; ?></textarea></div>
                                </div>
                                <div class="tr">
                                    <div class="cell"><strong>product Image:</strong> </div>
                                    
                                    <div class="cell"><input type="file" name="image" />
                                        <div><a href="<?php echo $upload_dir . $category_info['cover_image']; ?>" target="_BLANK" ><img style="margin-top:8px;height:150px;width: 300px;" src="<?php echo $upload_dir . $category_info['cover_image']; ?>" /></a></div>
                                    </div>
                                </div>
